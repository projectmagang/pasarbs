...
export class GetProvider {
  ...
  async loadData(){
    ...
    await this.http.get("http://192.168.1.13/barang/kategori_sayur.php", options)
    .map (res => res.json())
    .subscribe(data=>{
      this.datasayur = data;
    },err=>{
   ...
   // await this.http.get("http://192.168.1.12/pasarbs/src/app/pages/php/barang/kategori_buah.php", options)
    await this.http.get("http://192.168.1.13/barang/kategori_buah.php", options)
    .map (res => res.json())
    .subscribe(data=>{
      this.databuah = data;
    },err=>{
    ...
    await this.http.get("http://192.168.1.13/barang/databarang.php", options)
    .map (res => res.json())
    .subscribe(data=>{
